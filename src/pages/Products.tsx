...
import { Check, X, Sun, Thermometer, Zap, Clock, Shield, ArrowRight, AlertTriangle } from 'lucide-react';
...
const Products = () => {
  const features = [
    ...
    { feature: 'Grid Independence', hts: 'Partial', conventional: 'Full dependency' },
    { feature: 'Peak Load Reduction', hts: 'Significant', conventional: 'Minimal' },
  ];
  ...
  const requirements = [
    { title: 'Inverter AC Required', description: 'HTS panels work exclusively with inverter air conditioning units.', icon: Zap },
    { title: 'Outdoor Unit Access', description: 'The AC outdoor unit should be accessible near the terrace or rooftop.', icon: Sun },
    { title: 'Daytime Usage', description: 'Maximum benefits are achieved during daytime AC operation.', icon: Clock },
    { title: 'Professional Installation', description: 'Our certified technicians ensure optimal panel placement and connection.', icon: Shield },
  ];
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <p className="text-muted-foreground text-lg md:text-xl leading-relaxed mb-8">
                Revolutionary technology that combines thermal energy management with solar 
                efficiency to dramatically reduce your air conditioning costs while 
                ...
                {benefits.map((benefit, index) => (
                  ...
                  >
                    <div className="text-3xl font-bold text-primary">
                      {benefit.percentage}{benefit.label === 'Lifespan Years' ? '+' : '%'}
                    </div>
                    <div className="text-sm font-medium text-foreground">{benefit.label}</div>
                  </motion.div>
                ))}
              </div>
            </motion.div>
              ...
              <div className="aspect-square rounded-3xl bg-gradient-to-br from-primary/20 via-accent/10 to-secondary/20 p-8 relative overflow-hidden">
                <div className="absolute inset-4 rounded-2xl border-2 border-dashed border-primary/20 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <h3 className="font-outfit font-bold text-2xl text-foreground">HTS Technology</h3>
                    <p className="text-muted-foreground mt-2">Solar + Thermal Hybrid</p>
                  </div>
                </div>
          ...
          <table className="w-full glass-card rounded-2xl overflow-hidden">
            <thead>
              <tr className="bg-primary/10">
                <th className="text-left p-4 font-outfit font-semibold text-foreground">Feature</th>
                <th className="text-center p-4 font-outfit font-semibold text-primary">HTS Panels</th>
                <th className="text-center p-4 font-outfit font-semibold text-muted-foreground">Conventional Solar</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={row.feature} className={index % 2 === 0 ? 'bg-muted/30' : ''}>
                  ...
                  <td className="p-4 text-center">
                    {typeof row.hts === 'boolean' ? (
                      ...
                      <span className="text-primary font-semibold">{row.hts}</span>
                    )}
                  </td>
                  <td className="p-4 text-center">
      ...
      <Section background="gradient">
        ...
        >
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              HTS panels are specifically designed for use with inverter ACs. The outdoor unit 
              must be located near the terrace or rooftop for optimal installation. Our team 
              will assess your site to ensure compatibility before installation.
            </p>
          </div>
        </motion.div>
      </Section>
